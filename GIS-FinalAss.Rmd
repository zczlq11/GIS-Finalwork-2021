...
median_price <- merge_price   %>% 
  ...
  dplyr::summarise(mean_price=median(price))
mean_price<- merge_price %>%
  dplyr::group_by(GSS_CODE)%>%
  dplyr::summarise(mean_price=mean(price))
``` 

It shows that some wards don't have data, so I decide to add the April's Airbnb to the September's data.Because they're 90 days apart, there's no overlap in theory.

``` {r setup, results='hide'}
##it airbnb September data plus April data, "2015-listings.csv" is the sum 
InsideAirbnb3 <- read_csv("https://github.com/zczlq11/GIS-Finalwork-2021/blob/main/data/2015-listings.csv?raw=TRUE")
...
#get the mean_price by ward level
median_price3 <- merge_price3   %>% 
  dplyr::group_by(GSS_CODE)%>%
  dplyr::summarise(median_price=median(price))

```

In addition, 2020 Inside Airbnb dataset
```{r}
##it airbnb September data plus April data, "2015-listings.csv" is the sum 
InsideAirbnb2020 <- read_csv("http://data.insideairbnb.com/united-kingdom/england/london/2020-04-14/visualisations/listings.csv")
InsideAirbnb2020_sf <- st_as_sf(InsideAirbnb2020, 
                           coords = c("longitude","latitude"), 
                           crs = 4326) %>%
                           st_transform(., 27700)  
# get the rent price(<90 days)
rent_price90_2020 <- InsideAirbnb2020_sf  %>%
  filter(availability_365<=90) %>%
  dplyr::select("price", "availability_365","geometry") 

merge_price2020ward <- sf::st_join(Londonwards, rent_price90_2020["price"])

#get the mean_price by ward level
median_price2020ward <- merge_price2020ward%>%
  dplyr::group_by(GSS_CODE)%>%
  dplyr::summarise(median_price=median(price))
```
...
```{r setup, results='hide'}
crime <-read.csv("https://github.com/zczlq11/GIS-Finalwork-2021/blob/main/data/MPS_Ward_Level_Crime_Historic_NewWardfinal.csv?raw=TRUE", 
               header = TRUE, sep = ",", encoding = "latin1")

accessibility<-read.csv("https://github.com/zczlq11/GIS-Finalwork-2021/blob/main/data/Ward2011%20AvPTAI2015.csv?raw=TRUE", 
             header = TRUE, sep = ",", encoding = "latin1")

wardprofile2015<-read.csv("https://github.com/zczlq11/GIS-Finalwork-2021/blob/main/data/ward-profiles-excel-version.csv?raw=TRUE", 
                                               header = TRUE, sep = ",", encoding = "latin1")
variables <- crime %>%  
  left_join(., 
            accessibility,
